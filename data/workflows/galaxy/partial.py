...
def run_workflow(DATA_PATH):
	...
	preprocessed_images = glob.glob(DATA_PATH + "*.jpg")
	input_files  = []
	input_images = []
	for image_path in preprocessed_images:
		image_file = image_path.split("/")[-1]
		input_images.append(image_file)
		input_files.append(File(image_file))
	...
	train_class = 'train_class_'
	train_files = [File(i) for i in input_images if train_class in i]
	val_class = 'val_class_'
	val_files = [File(i) for i in input_images if val_class in i]
	test_class = 'test_class_'
	test_files = [File(i) for i in input_images if test_class in i]
	...
	vgg16_hpo_fn = 'vgg16_hpo_instrumented.py'
	...
	## CREATE WORKFLOW
	#---------------------------------------------------------------------------------------------------------
	wf = Workflow('Galaxy-Classification-Workflow-Without-Preprocessing')

	# job_create_dataset = Job(wrapper)\
	# 					.add_args("{} -seed {} --max_img {}".format(create_dataset_fn, SEED, 100))\
	...
	# job_preprocess_images = [Job(wrapper).add_args(preprocess_images_fn).add_inputs(preprocess_images_file) for i in range(NUM_PREPROCESSORS)]
	# resized_images = split_preprocess_jobs(job_preprocess_images, output_files, "_proc")
	
	# job_augment_class_2 = Job(wrapper)\
	# 					.add_args("{} --class_str class_2 --num {}".format(augment_images_fn, NUM_CLASS_2))\
	# 					.add_inputs(*input_aug_class_2, augment_images_file)\
	# 					.add_outputs(*output_aug_class_2)

	# job_augment_class_3 = Job(wrapper)\
	# 					.add_args("{} --class_str class_3 --num {}".format(augment_images_fn, NUM_CLASS_3))\
	...
	job_vgg16_hpo = Job(gpu_wrapper)\
						.add_args("{} --trials {} --epochs {} --batch_size {}".format(vgg16_hpo_fn, trials_per_worker, EPOCHS, BATCH_SIZE))\
						.add_inputs(*train_files, *val_files, data_loader_file, model_selection_file, vgg16_hpo_file)\
						.add_checkpoint(vgg16_pkl_file, stage_out=True)\
						.add_outputs(best_params_file, hpo_logdb_file)\
	...
	job_eval_model = Job(gpu_wrapper)\
						...
						.add_inputs(*test_files, data_loader_file, best_params_file,\
